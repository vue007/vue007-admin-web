<template>
    <div class="container">
        ...
        <el-table :data="state.list" v-loading="state.loadingTable">
            ...
            <el-table-column prop="headImg" label="头像" align="center" header-align="center" width="150" >
                <template slot-scope="scope">
                    <img style="width: 80px;width: 80px;border-radius: 50%" v-bind:src="scope.row.headImg" v-bind:alt="scope.row.userNickname" />
                </template>
            </el-table-column>
            <el-table-column prop="createTime" label="修改时间"  min-width="120" :formatter="$utils.dateTimeFormat"></el-table-column>

            <el-table-column label="审核" fixed="right" align="center" header-align="center" width="100" >
                <template slot-scope="scope">
                    <OperButton v-if="scope.row.status == $global.auditStatus.WAIT_AUDIT" content="通过" type="success" icon="yes"
                                oper="confirm"
                                confirmTitle="确定通过？"
                                confirmText="通过后该记录将生效！"
...
<script>
    ...
    const { mapState, mapActions } = createNamespacedHelpers('audit/headimg');
